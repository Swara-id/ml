...
data_folder = "./dataset/cv-corpus-17.0-2024-03-15/id/"
...
def mp3ToWav(filepath: str):
    print("Convert", filepath, "to wav")
    audio = AudioSegment.from_file(filepath)
    audio.export(filepath.replace(".mp3", ".wav"), format="wav")
...
def audioToTensor(filepath: str):
    ...
    spectrogram = (
        tf.math.log(tf.abs(tf.math.real(spectrogram)))
        / tf.math.log(tf.constant(10, dtype=tf.float32))
        * 20
    ) - 60
    spectrogram = tf.where(
        tf.math.is_nan(spectrogram), tf.zeros_like(spectrogram), spectrogram
    )
    spectrogram = tf.where(
        tf.math.is_inf(spectrogram), tf.zeros_like(spectrogram), spectrogram
    )
    return spectrogram
...
dataVoice, dataString, string_max_length = loadDataFromFile(
...
class MySequence(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        batch_x_string = self.x_string[
            idx * self.batch_size : (idx + 1) * self.batch_size
        ]
        batch_y_string = self.y_string[
            idx * self.batch_size : (idx + 1) * self.batch_size
        ]
        batch_x_voice = np.zeros(
            (self.batch_size, testParts.shape[0], testParts.shape[1])
        )
        for i in range(0, batch_size):
            batch_x_voice[i] = audioToTensor(
                os.path.join(clips_folder, self.x_voice[idx * self.batch_size + i])
            )
...
if os.path.exists(model_name) and reuse:
    ...
    split = tf.keras.layers.Reshape(
        (voice_max_length, -1, normalization.shape[2], normalization.shape[3])
    )(normalization)

    conv2d = Conv2D(34, 3, activation="relu")(split)
    conv2d = Conv2D(64, 3, activation="relu")(conv2d)
    maxpool = MaxPooling2D()(conv2d)
    dropout = Dropout(0.25)(maxpool)
...
model.fit(
    MySequence(X_voice, X_string, Y_string, batch_size),
    epochs=epochs,
